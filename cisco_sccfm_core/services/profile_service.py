...
class ProfileService:
    def __init__(self, path: Path | None = None) -> None:
        ...
        self._path = self._normalize_macos_path(selected_path)
    ...
    def remove(self, profile: str) -> bool:
        """Remove a named profile and report whether it existed."""
        self._validate_storage_path()
        self._ensure_parent_directory()
        self._validate_storage_path()
        if self._supports_posix_permissions():
            return self._remove_relative(profile)
        return self._remove_without_dir_fd(profile)
    ...
    def _save_relative(self, config: Profile) -> None:
    ...
    def _remove_relative(self, profile: str) -> bool:
        parent_descriptor = self._open_validated_parent_directory()
        try:
            profiles, original_stat = self._read_profiles_relative(parent_descriptor)
            ...
            self._write_relative_atomically(
                parent_descriptor,
                original_stat,
                {"profiles": profiles},
            )
    ...
    def _save_without_dir_fd(self, config: Profile) -> None:
        profiles, original_stat = self._read_profiles_without_dir_fd()
        payload = self._updated_payload(profiles, config)
        self._write_without_dir_fd(original_stat, payload)

    def _remove_without_dir_fd(self, profile: str) -> bool:
        profiles, original_stat = self._read_profiles_without_dir_fd()
        if profile not in profiles:
            return False
        del profiles[profile]
        self._write_without_dir_fd(original_stat, {"profiles": profiles})
        return True

    def _write_without_dir_fd(
        self,
        original_stat: os.stat_result | None,
        payload: Mapping[str, Any],
    ) -> None:
    ...
    def _require_descriptor_mode(descriptor: int, *, expected: int, label: str) -> None:
        ...
        ProfileService._require_mode(actual=actual, expected=expected, label=label)
