...
from cisco_sccfm_cli.commands.inventory.devices.ftd.shared import (
...
class FtdUpgradeCompatibleVersionsCommand(FtdDeviceTargetCommand):
    ...
    def handle(self, ctx: click.Context, **kwargs: Any) -> None:
        ...
        for uid, reason in results.skipped.items():
            ...
            label = f"{device.name} ('{uid}')" if device and device.name else f"'{uid}'"
        ...
        eligible_uid_to_device = {
            uid: targets.uid_to_device[uid]
            for uid in results.per_device
            if uid in targets.uid_to_device
        }

        self._render_results(
            results=results,
            uid_to_device=eligible_uid_to_device,
