...
"""Poll SCCFM until the newly registered FTD is ONLINE and has left NOT_SYNCED.

Asserts both end-to-end signals the user asked for:

* connectivity_state reaches ``ONLINE`` (the device phoned home), and
* config_state moves off the pre-registration ``NOT_SYNCED``/``NO_CONFIG`` value
  captured by the onboard phase (the manager configuration took effect).
"""
...
from cisco_sccfm_cli.e2e.ftd.phases.onboard_ftd import _CLI_KEY_STATE, _PRE_CONFIG_STATE
...
def _online_device(payload: Any) -> dict[str, Any] | None:
    """Return the single ONLINE device row, or None if not yet ready."""
...
def run(ctx: ProfileContext) -> None:
    ...
    pre_state = ctx.state.get(_PRE_CONFIG_STATE)
    ...
    try:
        for _ in range(FTD_REGISTRATION_RETRIES):
            ...
            if device is not None:
                ...
                post_state = device.get("config_state")
                # The device must have moved off its pre-registration state:
                # registration drives NOT_SYNCED -> (SYNCED / pending sync).
                assert post_state != pre_state, (
                    f"config_state did not change from {pre_state!r} after registration; "
                    f"device: {device!r}"
                )
                return
