...
"""List ASA devices not running a fake (non-existent) version."""

from __future__ import annotations

from cisco_sccfm_cli.e2e._profile import ProfileContext
from cisco_sccfm_cli.e2e._runner import get_json, run_cli
from cisco_sccfm_cli.e2e.asa.phases.test_data import ASA_NOT_ON_VERSION, ASA_TEST_QUERY
...
def run(ctx: ProfileContext) -> None:
    ...
    assert devices, f"Expected at least one device not on {ASA_NOT_ON_VERSION!r}, got {payload!r}"
    first = devices[0]
