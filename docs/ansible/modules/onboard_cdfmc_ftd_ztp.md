...
> MODULE [1mcisco.sccfm.onboard_cdfmc_ftd_ztp[0m (sccfm-ansible/plugins/modules/onboard_cdfmc_ftd_ztp.py)
...
[1mOPTIONS[0m (red indicates it is required):
   ...
   [1mfmc_access_policy_uid[0m  UUID of the FMC access policy to apply to
                          this device.
        type: str

   [1mlicenses[0m  List of licenses to apply to the device. At least one is
             required.
        choices: [BASE, CARRIER, THREAT, MALWARE, URLFilter]
        elements: str
        type: list

   [1mname[0m    Human-readable name for the FTD device.
        type: str
   ...
   [1mserial_number[0m  Serial number of the physical FTD device.
        type: str
