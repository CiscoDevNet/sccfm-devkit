...
DOCUMENTATION = r"""
---
module: add_network_group_members
short_description: Add referenced members to a network group in SCC Firewall Manager
description:
  - Add one or more referenced network objects to an existing network group.
...
options:
  ...
  referenced_objects:
    description: >-
      List of existing network object names or UIDs to add to the group.
...
network_group:
  description:
    - The resulting network group state.
...
def run_module() -> None:
    ...
    try:
        ...
        result: NetworkGroupMemberMutationResult = service.add_network_group_members(
            uid=uid,
            name=name,
        ...
        if module.check_mode:
            if result.changed:
                module.exit_json(
                    changed=True,
                    msg=f"Network group '{result.network_group.name}' would have members added.",
        ...
        if result.changed:
            module.exit_json(
                changed=True,
                msg=f"Successfully added members to network group '{result.network_group.name}'.",
                network_group=result.network_group.to_dict(),
            )
            return
        ...
        module.fail_json(msg=f"Failed to add network group members: {str(e)}")
