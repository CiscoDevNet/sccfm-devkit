...
import pytest
from plugins.modules import remove_network_group_members  # noqa: E402
...
@patch("plugins.modules.remove_network_group_members.Config")
@patch("plugins.modules.remove_network_group_members.NetworkGroupService")
@patch("plugins.modules.remove_network_group_members.AnsibleModule")
def test_should_remove_members_when_change_is_needed(
    ...
    updated_group = _make_group(refs=[])
    mutation_result = _make_mutation_result(updated_group, changed=True)

    mock_service = MagicMock()
    mock_service.remove_network_group_members.return_value = mutation_result
    mock_service_class.return_value = mock_service

    with pytest.raises(SystemExit):
        remove_network_group_members.run_module()

    mock_service.remove_network_group_members.assert_called_once_with(
        uid="net-grp-uid-456",
        name=None,
        referenced_objects=["ref-uid-001"],
        apply_changes=True,
    ...
    assert "Successfully removed members" in call_kwargs["msg"]


@patch("plugins.modules.remove_network_group_members.Config")
@patch("plugins.modules.remove_network_group_members.NetworkGroupService")
@patch("plugins.modules.remove_network_group_members.AnsibleModule")
def test_should_return_changed_false_when_members_already_absent(
    mock_ansible_module_class: MagicMock,
    mock_service_class: MagicMock,
    _mock_config_class: MagicMock,
    mock_module_instance: MagicMock,
    ...
    assert "already excludes all requested members" in call_kwargs["msg"]
...
def test_should_use_apply_changes_false_in_check_mode(
    ...
    call_kwargs = mock_module_instance.exit_json.call_args[1]
    assert call_kwargs["changed"] is True
    assert "would have members removed" in call_kwargs["msg"]
