...
def test_should_delete_network_group_by_uid(
    mock_ansible_module_class: MagicMock,
    mock_service_class: MagicMock,
    _mock_config_class: MagicMock,
    mock_module_instance: MagicMock,
) -> None:
    """run_module should delete a network group by UID and return changed=True."""
    mock_ansible_module_class.return_value = mock_module_instance

    mock_service = MagicMock()
    mock_service.delete_network_group.return_value = "net-grp-uid-123"
    mock_service_class.return_value = mock_service

    with pytest.raises(SystemExit):
        delete_network_group.run_module()

    mock_service.delete_network_group.assert_called_once_with(uid="net-grp-uid-123", name=None)
    mock_module_instance.exit_json.assert_called_once()
    call_kwargs = mock_module_instance.exit_json.call_args[1]
    assert call_kwargs["changed"] is True
    assert call_kwargs["deleted_uid"] == "net-grp-uid-123"
...
@patch("plugins.modules.delete_network_group.NetworkGroupService")
@patch("plugins.modules.delete_network_group.AnsibleModule")
def test_should_delete_network_group_by_name(
    mock_ansible_module_class: MagicMock,
    mock_service_class: MagicMock,
    _mock_config_class: MagicMock,
    mock_module_instance: MagicMock,
    ...
    """run_module should delete a network group by name and return changed=True."""
    mock_module_instance.params = base_module_params_with_name
    mock_ansible_module_class.return_value = mock_module_instance

    mock_service = MagicMock()
    mock_service.delete_network_group.return_value = "net-grp-uid-456"
    mock_service_class.return_value = mock_service

    with pytest.raises(SystemExit):
        delete_network_group.run_module()

    mock_service.delete_network_group.assert_called_once_with(uid=None, name="test-network-group")
    mock_module_instance.exit_json.assert_called_once()
    call_kwargs = mock_module_instance.exit_json.call_args[1]
    assert call_kwargs["changed"] is True
    assert call_kwargs["deleted_uid"] == "net-grp-uid-456"
    assert "Successfully deleted" in call_kwargs["msg"]
...
@patch("plugins.modules.delete_network_group.NetworkGroupService")
@patch("plugins.modules.delete_network_group.AnsibleModule")
def test_should_succeed_with_changed_false_when_group_not_found(
    mock_ansible_module_class: MagicMock,
    mock_service_class: MagicMock,
    _mock_config_class: MagicMock,
    mock_module_instance: MagicMock,
    ...
    """run_module should return changed=False when group is not found (idempotent)."""
    mock_module_instance.params = base_module_params_with_name
    mock_ansible_module_class.return_value = mock_module_instance

    mock_service = MagicMock()
    mock_service.delete_network_group.side_effect = NotFoundError(
        "Network group with name 'test-network-group' not found."
    )
    mock_service_class.return_value = mock_service
...
def test_should_fail_on_invalid_parameters(
    ...
    mock_service = MagicMock()
    mock_service.delete_network_group.side_effect = ValueError(
...
def test_should_fail_on_generic_exception(
    ...
    mock_service = MagicMock()
    mock_service.delete_network_group.side_effect = Exception(
...
def test_should_pass_all_parameters_to_service(
    ...
    mock_service = MagicMock()
    mock_service.delete_network_group.return_value = "resolved-uid-789"
    mock_service_class.return_value = mock_service

    with pytest.raises(SystemExit):
        delete_network_group.run_module()

    mock_service.delete_network_group.assert_called_once_with(
        uid=None,
        name="test-network-group",
    )
...
def test_build_argument_spec() -> None:
    ...
    spec = delete_network_group.build_argument_spec()
...
@patch("plugins.modules.delete_network_group.NetworkGroupService")
@patch("plugins.modules.delete_network_group.AnsibleModule")
def test_check_mode_should_report_would_delete_when_group_exists(
    mock_ansible_module_class: MagicMock,
    mock_service_class: MagicMock,
    _mock_config_class: MagicMock,
    mock_module_instance: MagicMock,
) -> None:
    """run_module in check_mode should report changed without deleting when group exists."""
    mock_module_instance.check_mode = True
    mock_ansible_module_class.return_value = mock_module_instance

    existing_group = MagicMock()
    existing_group.uid = "net-grp-uid-123"

    mock_service = MagicMock()
    mock_service.get_network_group.return_value = existing_group
    mock_service.get_network_group_by_name.return_value = existing_group
    mock_service_class.return_value = mock_service
    ...
    assert call_kwargs["deleted_uid"] == "net-grp-uid-123"
    mock_service.delete_network_group.assert_not_called()


@patch("plugins.modules.delete_network_group.Config")
@patch("plugins.modules.delete_network_group.NetworkGroupService")
@patch("plugins.modules.delete_network_group.AnsibleModule")
def test_check_mode_should_report_no_change_when_group_not_found(
    mock_ansible_module_class: MagicMock,
    mock_service_class: MagicMock,
    _mock_config_class: MagicMock,
    mock_module_instance: MagicMock,
) -> None:
    """run_module in check_mode should report changed=False when group does not exist."""
    mock_module_instance.check_mode = True
    mock_ansible_module_class.return_value = mock_module_instance

    mock_service = MagicMock()
    mock_service.get_network_group.return_value = None
...
def test_check_mode_should_fail_when_lookup_errors(
    ...
    mock_service = MagicMock()
    mock_service.get_network_group.side_effect = Exception("API unavailable")
    mock_service_class.return_value = mock_service
    ...
    mock_module_instance.fail_json.assert_called_once()
    call_kwargs = mock_module_instance.fail_json.call_args[1]
    assert "Failed to check network group existence" in call_kwargs["msg"]
