...
import pytest
from plugins.modules import get_access_rule  # noqa: E402
...
def test_should_get_access_rule_successfully(
    ...
    """run_module should fetch an access rule and return changed=False."""
    mock_ansible_module_class.return_value = mock_module_instance

    mock_service = MagicMock()
    mock_service.fetch_access_rule.return_value = sample_access_rule_response
...
def test_should_fail_on_api_error(
    ...
    mock_service = MagicMock()
    mock_service.fetch_access_rule.side_effect = Exception("API error: 404")
    mock_service_class.return_value = mock_service
