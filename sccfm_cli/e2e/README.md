...
- Credentials reuse the Ansible suite's vault (`scripts/setup_tokens.py`).  One CI bootstrap, two test surfaces.
- The FTD registration key is held only in the in-process phase store and is
  redacted from subprocess diagnostics before Jenkins or JUnit can record it.
...
4. (CI only) To run FTD registration, provide a pristine FTD through `FTD_HOST`,
   ...
   variable instead of `--cli-key` for the same reason.
...
```
poetry run pytest sccfm_cli/e2e/objects -v
poetry run pytest sccfm_cli/e2e/access_rules -v
poetry run pytest sccfm_cli/e2e/asa -v
poetry run pytest sccfm_cli/e2e/ftd -v
```

A single phase (parametrize id matches the phase name):

```
poetry run pytest "sccfm_cli/e2e/objects/test_network_object_lifecycle.py::TestNetworkObjectLifecycle::test_object_phase[create]" -v
poetry run pytest sccfm_cli/e2e/access_rules -v -k verify_create
```
...
```
SCCFM_E2E_ASA_UPGRADE_SOFTWARE_VERSION=9.6.4 SCCFM_E2E_ASA_UPGRADE_ASDM_VERSION=7.9.1 ./sccfm_cli/e2e/run_e2e.sh
SCCFM_E2E_FTD_UPGRADE_SOFTWARE_VERSION=7.4.2 ./sccfm_cli/e2e/run_e2e.sh
```
